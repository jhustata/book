...
qui { //grader has changed `if 1 to if c(os)'
	
	if c(os)=="MS Windows"  { //load data
	...
	if 5 { // question 5
		...
		*define program to output table 1
		...
		if c(os) == "MS Windows" { //grader created if c(os) block
		    ...
		    putexcel set "${HOME}/table1", replace 
		    putexcel A1="Question 5"
		    putexcel A2= "Age, median (IQR)"
		    putexcel A3 = "Previous transplant, %"
		}
		...
		*run program to make excel book
		...
		if c(os) == "MS Microsoft" {
		     import excel using "${HOME}\table1.xlsx", sheet("Sheet1")  clear

		}
		else  {
		     import excel using "${HOME}/table1.xlsx", sheet("Sheet1")  clear
	...
	if 6 { // question 6
		...
		if c(os) == "MS Office" {
		    ...
		    putexcel set "${HOME}/table2", replace 
		...
		if c(os) == "MS Windows" { //grader created if-blocks
		    import excel using "${HOME}\table2.xlsx", sheet("Sheet1")  clear
		}
		
		else { //ditto
		    import excel using "${HOME}/table2.xlsx", sheet("Sheet1")  clear
		}
	...
	if c(os) == "MS Windows" { // question 7; was if 7, grader... 
		...
		else { // question 7 ; inserted by grader 
