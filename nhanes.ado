...
program define nhanes 
	...
	preserve 
		...
		qui {
			...
			if 2 { //results: .do files
			
			    timer on 2
				
				clear
				do ${github}${$jhustata}nh3mort.do //mortality
				
				if c(edition_real) == "SE"  | c(edition_real) == "IC" {
					
					clear 
					do ${github}${$jhustata}ael-beta.do //survey
			...
			if 3 { //conclusions: only select vars 
